...
private:System::Void PaintChart()
{
			...
			for (int i = 0; i < this->选择需要显示的车次ToolStripMenuItem->DropDownItems->Count; i++)
			{
				...
				if (toolstripmenuitem->Checked == true)
				{
					//选择一种方案
					//在该方案中查询所有区间功耗
					String^selectsql = String::Format("SELECT 列车编号 AS 列车编号,区间编号 AS 区间编号,功耗 AS 功耗,运行时间 AS 运行时间 FROM 方案功耗数据表 " +
					...
					DataTable^powerdissipationdt = gcnew DataTable("同一方案各个车次功耗数据表");
					da->Fill(powerdissipationdt);
			...
			//查询最大区间号
			String^selectsql = String::Format("SELECT MAX(区间编号) AS 最大区间数 FROM 方案功耗数据表 " +
				"WHERE 时间 BETWEEN '{0}' AND dateadd(second,-1,dateadd(day,1,'{1}'));",
				this->dateTimePicker1->Value.ToShortDateString(), this->dateTimePicker1->Value.ToShortDateString());
			SqlDataAdapter^da = gcnew SqlDataAdapter(selectsql, con);
			DataTable^maxregionnumdt = gcnew DataTable("最大区间数表");
			da->Fill(maxregionnumdt);
			...
			for (int i = 0; i < regionnum; i++)//有regionnum个Series
			{
				...
				for (int j = 0; j < x_result_proposalsnum->Length; j++)//方案编号全发送
				{
					...
					//MessageBox::Show(y_result_runtime[j].ToString());
			...
			selectsql = String::Format("SELECT 方案编号,功耗 AS 最大功耗 FROM " +
				"(SELECT 方案编号 AS 方案编号,SUM(功耗) AS 功耗 FROM 方案功耗数据表 WHERE 时间 BETWEEN '{0}' AND dateadd(second,-1,dateadd(day,1,'{1}')) " +
			...
			da = gcnew SqlDataAdapter(selectsql, con);
			DataTable^maxpowerdissipationdt = gcnew DataTable();
			da->Fill(maxpowerdissipationdt);
			DataRow^dr1 = maxpowerdissipationdt->Rows[0];
			//MessageBox::Show(dr1["最大功耗"]->ToString());
			...
			da = gcnew SqlDataAdapter(selectsql, con);
			DataTable^minpowerdissipationdt = gcnew DataTable();
			da->Fill(minpowerdissipationdt);
			DataRow^dr2 = minpowerdissipationdt->Rows[0];
			radiuschart[2] = dr2["方案编号"]->ToString();
			radiuschart[3] = dr2["最小功耗"]->ToString();
			//MessageBox::Show(minpowerdissipationdt->Rows->Count.ToString());
			...
			da = gcnew SqlDataAdapter(selectsql, con);
			DataTable^avgpowerdissipationdt = gcnew DataTable();
			da->Fill(avgpowerdissipationdt);
	...
	private: System::Void PowerDissipationForm_Load(System::Object^  sender, System::EventArgs^  e) {
				 this->con->ConnectionString = "Data Source=localhost;Initial Catalog=电车数据库;Integrated Security=True";
				 con->Open();
				 String^selectsql = String::Format("SELECT DISTINCT 方案编号 AS 方案编号 FROM 方案功耗数据表 WHERE 时间 BETWEEN '{0}' AND dateadd(second,-1,dateadd(day,1,'{1}'));",
					 DateTime::Now.ToShortDateString(), DateTime::Now.ToShortDateString());
				 SqlDataAdapter^da = gcnew SqlDataAdapter(selectsql, con);
				 DataTable^proposalsnumdt = gcnew DataTable("方案表");
				 da->Fill(proposalsnumdt);
				 ...
				 selectsql = String::Format("SELECT DISTINCT 时间 AS 时间 FROM (SELECT DATEPART(year,时间) AS 时间 FROM 方案功耗数据表) AS 时间表;");
				 da = gcnew SqlDataAdapter(selectsql, con);
				 DataTable^yearsnumdt = gcnew DataTable();
				 da->Fill(yearsnumdt);
				 ...
				 //MessageBox::Show(yeardr["时间"]->ToString());
				 con->Close();
...
private: System::Void dateTimePicker1_ValueChanged(System::Object^  sender, System::EventArgs^  e) {
			 ...
			 //DataRow^dr = powerdt->Rows[0];
			 //MessageBox::Show(powerdt->Rows->Count.ToString());
...
private:System::Void ShowDaysPowerdissipation(int days)
{
			...
			{
				String^selectsql = String::Format("SELECT SUM(功耗) AS 当天功耗和 FROM 方案功耗数据表 WHERE 时间 BETWEEN '{0}' AND dateadd(second,-1,dateadd(day,1,'{1}'));",
				...
				SqlDataAdapter^da = gcnew SqlDataAdapter(selectsql, con);
				DataTable^powerdissipationdt = gcnew DataTable("各天功耗和表");
				da->Fill(powerdissipationdt);
			 ...
			 for (int i = 0; i < this->选择已有数据年份ToolStripMenuItem->DropDownItems->Count; i++)
			 {
				 ...
				 if (toolstripmenuitem->Checked == true)
				 {
					 String^sql = String::Format("SELECT DATEPART(month,时间) AS 月份,SUM(功耗) AS 当月功耗和 FROM " +
						 "(SELECT 功耗 AS 功耗,时间 AS 时间 FROM 方案功耗数据表 WHERE DATEPART(year,时间)={0}) AS T1 GROUP BY " +
...
private: System::Void webBrowser4_DocumentCompleted(System::Object^  sender, System::Windows::Forms::WebBrowserDocumentCompletedEventArgs^  e) {
			 String^selectsql = String::Format("SELECT DATEPART(year,时间) AS 年份,SUM(功耗) AS 当年功耗和 FROM 方案功耗数据表 GROUP BY DATEPART(year,时间);");
