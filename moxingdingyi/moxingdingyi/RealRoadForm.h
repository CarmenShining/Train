...
#pragma endregion
	...
	*/
	private: System::Void RealRoadForm_Load(System::Object^  sender, System::EventArgs^  e) {
		...
		//con1->Open();
		//selectsql = "UPDATE trainTable SET 方案编号='test190615' WHERE 方案编号='test' AND 线路编号='001';";
		//selectsql = "DELETE FROM trainTable WHERE 方案编号='ctest1';";
		//SqlCommand^cmd = gcnew SqlCommand(selectsql, con1);
		//cmd->ExecuteNonQuery();
		//con1->Close();
	}
			 //选择城市
	...
	*/
	...
	private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) {
		if (this->MakeprojectradioButton1->Checked == true)
		{
			...
			/*
			//在richtextbox中显示运行时间表信息
			for (int i = 0; i < starttime_arr->Count; i++)
			{
				String^carnum_1 = op->trams[map_starttimetotrams[i]]->tram_strnum;
				String^time_car = starttime_arr[i].TimeOfDay.ToString();
				String^direction_car;
				if (op->trams[map_starttimetotrams[i]]->direction == 0)
				{
					direction_car = "上冲小镇->水拥坑";
				}
				else if (op->trams[map_starttimetotrams[i]]->direction == 1)
				{
					direction_car = "水拥坑->上冲小镇";
				}
				String^cars_table = carnum_1 + "\t" + time_car + "\t" + direction_car + "\n";
				this->richTextBox1->AppendText(cars_table);
			}*/
		}
		else if (this->RepeatradioButton2->Checked == true)// **************************************
		{
	...
	private: System::Void timer1_Tick(System::Object^  sender, System::EventArgs^  e) {
		...
		if (this->MakeprojectradioButton1->Checked == true)//模拟多车方案运行**********
		{
			for (int i = 0; i < op->trams->Count; i++)
			{
				...
				if (op->trams[i]->draw)
				{
					array<System::Object^>^createcar = gcnew array<System::Object^>(6);
					createcar[5] = op->trams[i]->peopontram;//车内人数
		...
		else if (this->RepeatradioButton2->Checked == true)//重复模式***********
		{
			for (int i = 0; i < op->trams->Count; i++)
			{
				op->Run_imm(op->trams[i], i);
		...
		else if (this->CurrentradioButton1->Checked == true)//实时运行模式***************
		{
			op->SysRun(op->trams[0]);
			if (op->trams[0]->draw)
			{
				array<System::Object^>^createcar = gcnew array<System::Object^>(6);
				//createcar[5] = op->trams[0]->peopontram;//车内人数
				createcar[5] = op->trams[0]->passedsitenum;//经过正向站点数
				//createcar[4] = op->trams[0]->velocity;//速度
				createcar[4] = op->trams[0]->inv_passedsitenum;//经过反向站点数
