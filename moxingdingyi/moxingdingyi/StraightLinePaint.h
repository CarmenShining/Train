	...
	public:
		...
		System::Void initLine() {
			...
			while (site_index < sites->Count)
			{
				...
				sites1[site1_index]->pos = gcnew Point(site_dist - orbit_len, -0.1f);     //获取站点绘制坐标
				/*
			...
			for (int i = 0; i < sites->Count; i++)
			{
				for (int j = 0; j < trams->Count; j++) {
					...
					if (trams[j]->positive_run) {
						//
						if (trams[j]->p5->_x >= sites[i]->pos->_x - out_R && trams[j]->p5->_x <= sites[i]->pos->_x + out_R && trams[j]->hasPassed == i && trams[j]->hasPassed<(sites->Count-1) && sites[i]->ifIn==false) {
							sites[i]->ifArrived = true;//列车到站
							sites[i]->arrivedTram_no = j;
							sites[i]->arrivedTram_no1 = trams[j]->tram_no;
							sites[i]->arrivedTram_strnum = trams[j]->tram_num;
							trams[j]->velocity = 0;//更新列车速度，站点停留
							trams[j]->hasPassed++;
						}
						else if (!trams[j]->finish && trams[j]->hasPassed>0 && trams[j]->p5->_x>sites[trams[j]->hasPassed-1]->pos->_x + out_R && sites[trams[j]->hasPassed]->ifArrived==false)
						{
					...
					//反向
						...
						if (trams[j]->p5->_x <= sites1[sites1->Count-1-i]->pos->_x + out_R && trams[j]->p5->_x >= sites1[sites1->Count - 1 - i]->pos->_x - out_R && trams[j]->hasPassed == i && trams[j]->hasPassed<(sites1->Count-1) && sites1[sites1->Count - 1 - i]->ifIn==false) {
							...
							trams[j]->velocity = 0;
							//MessageBox::Show(trams[j]->hasPassed.ToString());
							trams[j]->hasPassed++;
						}
						else if (!trams[j]->finish && trams[j]->hasPassed > 0 && trams[j]->p5->_x < sites1[sites1->Count - 1 - (trams[j]->hasPassed - 1)]->pos->_x - out_R && sites1[sites1->Count - 1 - trams[j]->hasPassed]->ifArrived == false)
						{
							sites1[sites1->Count - 1 - (trams[j]->hasPassed-1)]->ifIn = true;
							sites1[sites1->Count - 1 - i]->arrivedTram_strnum = trams[j]->tram_num;
						}
					}
				}
		...
		System::Void showStand() 
		{
			...
			for (int site_index = site_num - 1; site_index >= 0; site_index=site_index-1) {
				...
				//绘制列车到站图标
				if (sites1[site_index]->ifArrived && sites1[site_index]->ifIn==false) {
					glColor3f(1.0f, 0.0f, 0.0f); //设置颜色
					glBegin(GL_POLYGON);//开始画圆
					for (int i = 0; i < 20; ++i)
				...
				//绘制列车站间图标
				if (site_index > 0 && sites1[site_index]->ifIn && !sites1[site_index-1]->ifArrived) {
					glColor3f(1.0f, 0.0f, 0.0f); //设置颜色
					glBegin(GL_POLYGON);//开始画圆
					for (int i = 0; i < 20; ++i)
