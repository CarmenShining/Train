...
#pragma endregion
	...
	private:System::Void PaintChart1()
	{
				...
				//MessageBox::Show(y_arrht->Keys->Count.ToString());
	...
	private: System::Void PeopleFlowForm_Load(System::Object^  sender, System::EventArgs^  e) {
				 ...
				 con->ConnectionString = "Data Source=localhost;Initial Catalog=电车数据库;Integrated Security=True";

				 String^selectsql = String::Format("SELECT 方案编号 AS 方案编号,MAX(站点数) AS 最大站点数 FROM " +
					 "(SELECT 方案编号 AS 方案编号,COUNT(站点名称) AS 站点数 FROM 线路人流量数据表 GROUP BY 方案编号) " +
					 "AS T1 GROUP BY 方案编号;");
				 DataTable^stationsnumdt = GetDataTable(con, selectsql);
				 DataRow^dr1 = stationsnumdt->Rows[0];
				 maxstationsnum = int::Parse(dr1["最大站点数"]->ToString());

				 selectsql = String::Format("SELECT DISTINCT 站点名称 AS 站点名称,站点编号 AS 站点编号 FROM 线路人流量数据表 " +
					 "WHERE 方案编号='{0}' ORDER BY 站点编号;", dr1["方案编号"]->ToString());
				 DataTable^stationsdt = GetDataTable(con, selectsql);
				 x_result = gcnew array<String^>(maxstationsnum);
				 for (int i = 0; i < stationsdt->Rows->Count; i++)
				 {
					 DataRow^dr = stationsdt->Rows[i];
					 //MessageBox::Show(dr["站点名称"]->ToString());
					 x_result[i] = dr["站点名称"]->ToString();
				 }
				 //MessageBox::Show(dr1["最大站点数"]->ToString());
	}
			 //选择日期改变
private: System::Void dateTimePicker1_ValueChanged(System::Object^  sender, System::EventArgs^  e) {
			 String^selectsql = String::Format("SELECT DISTINCT 列车编号 AS 列车编号,方案编号 AS 方案编号 FROM 线路人流量数据表 " +
...
private: System::Void 勾选方案ToolStripMenuItem_CheckedChanged(System::Object^  sender, System::EventArgs^  e) {
			 ...
			 for (int i = 0; i < this->勾选方案ToolStripMenuItem->DropDownItems->Count; i++)
			 {
				 ...
				 if (toolstripmenuitem->Checked == true)
				 {
					 ...
					 String^selectsql = String::Format("SELECT 上车人数 AS 上车人数,下车人数 AS 下车人数,站点名称 AS 站点名称,站点编号 AS 站点编号 " +
						 "FROM 线路人流量数据表 WHERE 方案编号='{0}' AND 时间 BETWEEN '{1}' AND dateadd(second,-1,dateadd(day,1,'{2}')) ORDER BY 站点编号;",
...
private:System::Void ShowPeopleFlow(int days)
{
			...
			{
				String^selectsql = String::Format("SELECT SUM(上车人数) AS 上车人数和,SUM(下车人数) AS 下车人数和 FROM 线路人流量数据表 WHERE 时间 BETWEEN '{0}' AND dateadd(second,-1,dateadd(day,1,'{1}'));",
