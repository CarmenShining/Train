...
private: System::Void RoadChoiceChanged(System::Object^  sender, System::EventArgs^  e) {
	...
	private: System::Void toolStripMenuItem_CheckedChanged(System::Object^  sender, System::EventArgs^  e)
	{
		...
		if (toolitem->Checked == true)
		{
			...
			bool dirflag = false;
			//MessageBox::Show(dirtb->Rows[0]->ItemArray[0]->ToString()->Trim());
			//MessageBox::Show("正向");
			...
			//int numf = tram_to_index->Keys->Count+1;
			//tram_to_index->Add(toolitem->Text->ToString(), numf);
		}
		else
		{
	...
	private:System::Void SysMode_ItemCheckedChanged(System::Object^  sender, System::EventArgs^  e)
	{
		...
		if (item->Checked == true)
		{
			...
			if (item->Checked == true)
			{
				...
				//int numf = tram_to_index->Keys->Count + 1;
				//tram_to_index->Add(item->Text->ToString(), numf);
...
private:System::Void UpdateTramInfo()
{
	...
	this->EmptyRatetextBox3->Text = OpenGL->trams[t_index]->rate.ToString()->Trim();
	if (OpenGL->trams[t_index]->positive_run)
		this->NextStationtextBox4->Text = OpenGL->sites[OpenGL->trams[t_index]->hasPassed]->name->Trim();
	else
		this->NextStationtextBox4->Text = OpenGL->sites1[OpenGL->sites1->Count-1 - (OpenGL->trams[t_index]->hasPassed)]->name->Trim();
	this->PostextBox5->Text = OpenGL->trams[t_index]->pos.ToString();
	...
	this->VoltagetextBox9->Text = OpenGL->trams[t_index]->voltage.ToString()->Trim();
	this->VelocitytextBox10->Text = OpenGL->trams[t_index]->velocity.ToString()->Trim();
	this->AcceleratetextBox11->Text = OpenGL->trams[t_index]->accelerate.ToString()->Trim();
	if (OpenGL->trams[t_index]->finish)
		this->iffinishlabel22->BackColor = System::Drawing::Color::Lime;
	else
		this->iffinishlabel22->BackColor = System::Drawing::Color::Red;
	//绘制速度曲线图
	String^selectsql = "SELECT cast(时间 as datetime),cast(速度 as float) FROM trainTable WHERE 数据序号>'"+ (OpenGL->trams[t_index]->datanum-5).ToString()+"' AND "+
